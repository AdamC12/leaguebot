...
def get_summoner_puuid(summoner_name)
  request = "https://euw1.api.riotgames.com/lol/summoner/v4/summoners/by-name/#{summoner_name}?api_key=RGAPI-3a7292dd-fca9-442f-a0ec-4392cf95b9c5"
  response = HTTParty.get(request)
  puuid = response['puuid']
  return puuid
end

def get_soloq_matches_for_puuid(puuid)
  request = "https://europe.api.riotgames.com/lol/match/v5/matches/by-puuid/#{puuid}/ids?queue=420&start=0&count=20&api_key=RGAPI-3a7292dd-fca9-442f-a0ec-4392cf95b9c5"
  response = HTTParty.get(request)
  @matches = response
end
...
def get_unique_champions(summoner)
  ...
  @matches.each do |match|
    request = "https://europe.api.riotgames.com/lol/match/v5/matches/#{match}?api_key=RGAPI-3a7292dd-fca9-442f-a0ec-4392cf95b9c5"
    response = HTTParty.get(request)
    champion_list << response['info']['participants'].detect{|h| h['summonerName'] == summoner}['championName']
  end

  return champion_list.uniq.count
end
